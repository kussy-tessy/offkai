<template>
  <section class="space-y-5">
    ...
    <template v-else-if="page">
      <div class="flex flex-wrap items-center justify-between gap-3 rounded-lg border px-3 py-3" :class="page.settlementLockedAt
        ...
        <MyButton v-if="canConfirm && !page.settlementLockedAt" size="sm"
          :disabled="saving !== null || !page.feeCalculationLockedAt" @click="lockDialogOpen = true">経費精算を確定</MyButton>
        <MyButton v-else-if="canConfirm && !page.refundStartedAt" size="sm" color="gray" variant="ghost"
          :disabled="saving !== null" @click="unlockDialogOpen = true">確定を解除</MyButton>
        <span v-else-if="page.refundStartedAt" class="text-xs font-medium text-emerald-800">返金開始済み・解除不可</span>
      </div>
      <div v-if="!page.feeCalculationLockedAt"
        class="rounded-lg border border-amber-200 bg-amber-50 px-3 py-3 text-sm text-amber-800">
        <strong>参加費が未確定です。</strong>
      ...
      <label class="block text-sm text-slate-600">
        返金額の切り捨て単位
        <select :value="page.refundRoundingUnit"
          ...
          :disabled="!canConfirm || page.settlementLockedAt !== null || saving !== null" @change="updateRoundingUnit">
          <option :value="10">10円単位</option>
          <option :value="100">100円単位</option>
          <option :value="500">500円単位</option>
        </select>
      </label>
      <div v-if="page.categories.length === 0"
      ...
      <div v-else class="space-y-4">
        <SettlementCategoryPanel v-for="category in page.categories" :key="category.id" :category="category"
          :participants="page.participants" :initially-open="false" :saving-action="saving"
          :locked="!canEdit || page.settlementLockedAt !== null" :save-expense="saveExpense"
          :delete-expense="deleteExpense" :save-income="saveIncome" :delete-income="deleteIncome" />
      </div>
      <ParticipantRefundCalculationSection
        :participants="page.participants"
        :categories="page.categories"
        :locked="!canEdit || page.settlementLockedAt !== null"
        :saving="saving !== null"
        :save-note="saveParticipantNote"
      />
    </template>
    <MyConfirmDialog v-model:open="lockDialogOpen" title="経費精算を確定しますか？"
      message="確定時の内容で参加者ごとの返金額を保存します。返金を始める前であれば確定解除できます。" confirm-label="経費精算を確定" :loading="saving === 'lock'"
      @confirm="lockSettlement" />
    <MyConfirmDialog v-model:open="unlockDialogOpen" title="経費精算の確定を解除しますか？"
      message="保存済みの返金額はクリアされ、経費・収入を再び編集できるようになります。" confirm-label="確定を解除" confirm-color="red"
      :loading="saving === 'unlock'" @confirm="unlockSettlement" />
  </section>
</template>
...
<script setup lang="ts">
  ...
  import ParticipantRefundCalculationSection from "./ParticipantRefundCalculationSection.vue";
  import type {
  ...
  const saveParticipantNote = async (userId: string, note: string | null) => {
    if (saving.value) return false;
    saving.value = "participant-note";
    try {
      const result = await put<SettlementPage>(
        `${basePath}/settlement/participants/${userId}/note`,
        { note },
      );
      if (!result) throw new Error();
      page.value = result;
      toast.success("経費精算時の備考を保存しました。");
      return true;
    } catch (cause) {
      toast.error(getApiErrorMessage(cause, "経費精算時の備考を保存できませんでした。"));
      return false;
    } finally {
      saving.value = null;
    }
