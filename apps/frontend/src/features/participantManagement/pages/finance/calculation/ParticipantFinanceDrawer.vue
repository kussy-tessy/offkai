    ...
    <div v-if="participant" class="fixed inset-0 z-40" @keydown.esc="onClose">
      ...
      >
        ...
        >
          ...
          <button
            class="rounded-md px-3 py-1 text-2xl text-slate-500 hover:bg-slate-100"
            aria-label="閉じる"
            @click="onClose"
          >
            ×
        ...
        <div class="flex-1 space-y-7 overflow-y-auto px-5 py-5">
          <section>
            ...
            <div
              ...
              class="mt-3 rounded-lg bg-slate-50 px-4 py-5 text-sm text-slate-500"
            >
              精算区分がありません。
            </div>
            <div
              ...
              >
                <div class="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <span class="text-sm text-slate-500"
                    >基本 {{ money(category.baseParticipationFeeAmount) }}</span
                  >
                </div>
                <div
                  v-if="memberOf(category.id)"
                  class="flex flex-wrap items-end gap-2 pl-6"
                >
                  <label class="min-w-48 flex-1 text-sm text-slate-600">
                    個別金額（空欄なら基本金額）
                    <input
                      :value="overrideDrafts[category.id] ?? ''"
                      type="number"
                      min="0"
                      step="1"
                  ...
                  <MyButton
                    ...
                    variant="ghost"
                    :disabled="saving || locked"
                    @click="saveOverride(category.id)"
                    >反映</MyButton
                  >
                </div>
              </div>
          ...
          <section>
            ...
            <div
              v-if="participant.extraCharges.length"
              class="mt-3 divide-y divide-slate-100 rounded-lg border border-slate-200"
            >
              <div
                v-for="charge in participant.extraCharges"
                :key="charge.id"
              ...
              >
                ...
                <button
                  class="rounded p-1.5 text-rose-600 hover:bg-rose-50"
                  :disabled="saving || locked"
                  title="削除"
                  :aria-label="`${charge.title}を削除`"
                  @click="removeExtra(charge.id)"
                >
            ...
            <div
              class="mt-3 grid gap-3 rounded-lg bg-slate-50 p-4 sm:grid-cols-2"
            >
              <label class="text-sm text-slate-600"
                >内容<input
                  v-model="extraDraft.title"
                  :disabled="locked"
              ...
              <div class="sm:col-span-2">
                <MyButton
                  size="sm"
                  :loading="savingAction === 'extra'"
                  :disabled="saving || locked || !canAddExtra"
                  @click="addExtra"
                  >追加</MyButton
                >
              </div>
          ...
          <section
            class="rounded-xl bg-gradient-to-r from-teal-50 to-sky-50 p-4"
          >
            ...
            <dl class="mt-3 space-y-2 text-sm">
              ...
              <div
                class="flex justify-between gap-4 border-t border-teal-200 pt-3 text-base"
              >
