        ...
        <thead>
          <tr class="bg-slate-100">
            <th
              v-for="h in headers"
              :key="h.key"
              class="whitespace-nowrap border border-slate-300 px-2 py-1 text-left"
            >
              {{ h.label }}
            </th>
          </tr>
        ...
        <tbody>
          ...
          >
            <td
              ...
              :key="cell.key"
              class="whitespace-nowrap border border-slate-300 px-2 py-1 align-top"
            >
              <template v-if="cell.key === 'name'">
                <span>{{ cell.value }}</span>
                <button
                  v-if="data.canEditAnswers"
                  type="button"
...
const headers = computed(() =>
  data.value
    ? [
        { key: "name", label: "名前" },
        ...data.value.commitmentQuestions.map((q) => ({
          key: `c-${q.id}`,
          label: q.question,
        })),
        ...data.value.preferenceQuestions.map((q) => ({
          key: `p-${q.id}`,
          label: q.question,
        })),
        ...(data.value.askBringingKigurumi
          ? [{ key: "kigurumi", label: "連れてくる着ぐるみさん" }]
          : []),
      ]
    : [],
);
const rowCells = (p: Participant) =>
  data.value
    ? [
        ...
        ...data.value.commitmentQuestions.map((q) => ({
          key: `c-${q.id}`,
          value:
            p.commitmentAnswers[q.id] === "yes"
              ? "はい"
              : p.commitmentAnswers[q.id] === "no"
                ? "いいえ"
                : "",
        })),
        ...data.value.preferenceQuestions.map((q) => ({
          key: `p-${q.id}`,
          value: p.preferenceAnswers[q.id] ?? "",
        })),
        ...(data.value.askBringingKigurumi
          ? [
              {
                key: "kigurumi",
                value: p.bringingKigurumis
                  .map((k) => `${k.character}（${k.title}）`)
                  .join(" / "),
              },
            ]
          : []),
...
const copyTable = async () => {
  ...
  const t = [
    headers.value.map((h) => safe(h.label)).join("\t"),
    ...data.value.participants.map((p) =>
      rowCells(p)
        .map((c) => safe(c.value))
        .join("\t"),
    ),
...
const load = async () => {
  ...
  try {
    const result = await get<Data>(
      `/offkai-event/${eventId}/participant-answer-table`,
    );
    if (!result) throw new Error("回答表を取得できませんでした。");
    data.value = result;
  } catch (cause) {
