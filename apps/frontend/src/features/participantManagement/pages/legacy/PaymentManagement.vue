    ...
    >
      <table class="w-full min-w-max border-collapse text-sm">
        ...
        >
          <tr>
            <th
              class="sticky left-0 z-20 w-28 min-w-28 bg-teal-50 px-2 py-2 text-left"
            >
              名前
            </th>
            <th
          ...
          >
            ...
            >
              ...
              <span
                v-else
                class="text-gray-400"
                :aria-label="`${question.questionShort}：未回答`"
                >―</span
              >
            </td>
            <td
              class="border-b border-slate-100 px-2 py-2 text-center align-middle"
            >
              <PaymentAmountInput
                ...
                :value="row.amount"
                :disabled="
                  row.savingField !== null && row.savingField !== 'amount'
                "
                :save="(amount) => saveRow(row, { amount }, 'amount')"
                @navigate="
                  (direction) => focusRelativeAmount(rowIndex, direction)
                "
              />
              <p
                v-if="row.savingField === 'amount'"
                class="mt-1 text-xs text-teal-700"
              >
                保存中…
              </p>
              <p v-else-if="row.saved" class="mt-1 text-xs text-emerald-700">
                保存しました
              </p>
              <p v-if="row.error" class="mt-1 max-w-36 text-xs text-red-600">
                {{ row.error }}
              </p>
            </td>
            <td
            ...
            >
              <MyAsyncCheckbox
                ...
                :save="
                  (value) => saveRow(row, { collected: value }, 'collected')
                "
              />
            </td>
          </tr>
...
const SHOW_COMMITMENT_STORAGE_KEY =
  "participant-payments:show-commitment-answers";
...
type Participant =
  Unbrand<GetParticipantPaymentsResponse>["participants"][number];
...
const loadError = ref("");
const questions = ref<
  Unbrand<GetParticipantPaymentsResponse>["commitmentQuestions"]
>([]);
...
const amountInputId = (userId: string) =>
  `participant-payment-amount-${userId}`;
