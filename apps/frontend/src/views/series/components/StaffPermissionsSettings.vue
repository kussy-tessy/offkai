<template>
  <section class="space-y-6 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
    <div>
    ...
    <template v-else>
      <PermissionSection title="参加前の権限">
        <MyCheckbox :value="permissions.showUnansweredEvents"
          :on-change="(v) => (permissions.showUnansweredEvents = v)">
          参加表明していないオフ会もダッシュボードに表示する
        </MyCheckbox>
        <MyCheckbox :value="permissions.allowApplicationBeforeStart"
      ...
      <PermissionSection title="オフ会の管理">
        <MyCheckbox :value="permissions.eventManagement" :on-change="(v) => (permissions.eventManagement = v)">
          オフ会の内容を編集できる
        </MyCheckbox>
        <p class="text-xs text-slate-500">
          アンケート、締切、定員、募集開始日時、概要、公開範囲、参加者向け案内などを変更できます。オフ会の作成・削除は代表者のみ可能です。
        </p>
      </PermissionSection>

      <PermissionSection title="参加者管理">
        <PermissionChoices v-model="permissions.answerManagement" label="ほかの参加者の回答" :options="answerOptions" />
      </PermissionSection>

      <PermissionSection title="Discordロール">
        <PermissionChoices v-model="permissions.discordRole" label="Discordロール" :options="discordOptions" />
      </PermissionSection>

      <PermissionSection title="お金の管理">
        <PermissionChoices v-model="permissions.feeCalculation" label="参加費計算" :options="feeCalculationOptions" />
        <PermissionChoices v-model="permissions.feeCollection" label="参加費徴収" :options="feeCollectionOptions" />
        <PermissionChoices v-model="permissions.settlement" label="経費精算" :options="settlementOptions" />
        <PermissionChoices v-model="permissions.refund" label="返金" :options="refundOptions" />
      </PermissionSection>
      ...
      <div
        ...
        :class="isDirty ? 'border-amber-300 bg-amber-50' : 'border-slate-200 bg-white'
          ">
        <p class="text-sm font-medium" :class="isDirty ? 'text-amber-900' : 'text-slate-500'">
          {{ isDirty ? "未保存の変更があります。" : "変更はありません。" }}
        </p>
        <MyButton class="w-full sm:w-auto" :loading="saving" :disabled="saving || !isDirty" @click="save">
          変更を保存する
        </MyButton>
      </div>
    </template>

    <MyConfirmDialog v-model:open="leaveConfirmOpen" title="未保存の変更を破棄しますか？"
      message="スタッフ権限への変更がまだ保存されていません。移動すると変更内容は失われます。" confirm-label="変更を破棄して移動" cancel-label="編集を続ける"
      confirm-color="red" @confirm="confirmLeave" @cancel="cancelLeave" />
  </section>
</template>
