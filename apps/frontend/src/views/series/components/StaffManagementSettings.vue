  ...
  <section class="space-y-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
    <div>
      ...
      <p class="mt-1 text-sm text-slate-600">
        このオフ会の運営スタッフを管理します。KigPlaにDiscord連携済みで、このオフ会のDiscordサーバーに参加しているユーザーを追加できます。
      </p>
    </div>
    ...
    <form class="flex flex-col gap-3 sm:flex-row sm:items-start" @submit.prevent="addStaff">
      <div class="min-w-0 flex-1">
        ...
        <input id="staff-discord-username" v-model.trim="discordUsername" type="text" autocomplete="off"
          autocapitalize="none" spellcheck="false" placeholder="例: username" :disabled="adding"
    ...
    <MyConfirmDialog v-model:open="confirmRemovalOpen" title="スタッフから削除しますか？"
      :message="removalTarget ? `${removalTarget.userName} さんをこのオフ会の運営スタッフから削除します。` : ''" confirm-label="削除する"
      confirm-color="red" :loading="removing" @confirm="removeStaff" />
  </section>
</template>
