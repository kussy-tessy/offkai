<template>
  <main class="space-y-6">
    <MyBackLink :to="`/offkai/${id}/detail`">回答一覧へ戻る</MyBackLink>
    ...
    <section v-else class="space-y-3">
      <h2 class="text-xl font-semibold text-slate-800">Discord管理</h2>
      <div class="overflow-hidden rounded-xl border border-teal-100 shadow-sm">
        <table class="w-full border-collapse table-fixed">
          <thead class="bg-gradient-to-r from-teal-50 to-sky-50 border-b border-teal-100">
            <tr>
              <th class="p-3 text-left w-32">名前</th>
              <th class="p-3 text-left">DiscordId</th>
              <th class="p-3 text-center w-36">ロール割り当て</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="member in members" :key="member.userId" class="odd:bg-white even:bg-slate-50/70">
              ...
              <td class="border-b border-slate-100 p-3 text-sm text-slate-600">
                <div class="flex items-center gap-2">
                  <img v-if="member.discordAvatarUrl" :src="member.discordAvatarUrl" alt="" class="h-6 w-6 rounded-full" loading="lazy" />
              ...
              <td class="border-b border-slate-100 p-3 text-center">
                <MyAsyncCheckbox :value="member.hasRole" :disabled="!member.canManageRole"
...
<script setup lang="ts">
  ...
  const fetchMembers = async () => {
    ...
    loadError.value = "";
    try {
      const result = await get<GetOffkaiEventDiscordRoleMembersResponse>(
        `/offkai-event/${id}/discord-role-members`,
      );
      data.value = result;
    } catch (cause) {
      loadError.value = getApiErrorMessage(cause, "参加者管理情報の読み込みに失敗しました。");
    } finally {
      loading.value = false;
    }
  };
  ...
  const saveRole = async (userId: string, hasRole: boolean) => {
    await put<UpdateOffkaiEventDiscordRoleMemberResponse>(
      `/offkai-event/${id}/discord-role-members/${userId}`,
      { hasRole },
    );
  };
