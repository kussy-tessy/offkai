<template>
  ...
  <OffkaiOverview v-else-if="detail" :data="detail" />
</template>
...
import OffkaiOverview from "@/features/offkaiDetail/components/OffkaiOverview.vue";

const { id } = defineProps<{ id: string }>();
const { get } = useApi();
const router = useRouter();
const detail = ref<OffkaiDetail | null>(null);
const loadError = ref("");
